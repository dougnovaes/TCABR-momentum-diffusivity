...
%   This function computes theoretical predictions for:
%   1. Toroidal Velocity (Helander Model)
%   2. Momentum Diffusivity and Pinch Velocity (Solomon et al. 2010 Model)
%
%   Syntax:
%       theoretical_models = compute_theoretical_models(temp_results, mag_field, derived_profiles, constants)
    ...
    % Use a small, physically-motivated offset for B_pol to avoid singularity
    B_pol_offset  = B_pol + 0.0008; % Offset justified by Shafranov shift
    ...
    % Diffusivity for R/Ln = 2 (reference value)
    ...
    % Diffusivity for R/Ln calculated at mid-radius
    ...
    % --- 4. Package Results into Nested Structure ---
    theoretical_models.Vphi.Helander = Vphi_Helander;
    theoretical_models.Vpinch.Solomon = Vpinch_solo;
    theoretical_models.chi_phi.Solomon_Rln2 = chi_phi_solo_Rln2;
    theoretical_models.chi_phi.Solomon_Rln_calc = chi_phi_solo_Rln_calc;
    theoretical_models.meta.R_over_Ln_mid_value = R_over_Ln_mid;

    fprintf('Theoretical model profiles calculated successfully.\n\n');
end

function dTi_dr = analytical_dTi_dr(params, r, a)
    % Calculates the analytical derivative of the canonical Ti profile.
