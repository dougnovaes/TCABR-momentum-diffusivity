...
%       derived_profiles - A structure with nested fields containing all profiles.
    ...
    % --- 5a. Wesson Collisionality (based on bounce frequency) ---
    fprintf('... calculating Wesson collisionality\n');
    epsilon = constants.machine.epsilon_aspect_ratio;
    omega_bounce_e = sqrt(epsilon) .* thermal_velocities.v_th_e ./ (q_and_shear.q_profile .* R_coord);
    collisionality.nu_star_e_wesson = collisions.nu_ei_15 ./ (epsilon .* omega_bounce_e);

    % --- 5b. Solomon/Maslov Collisionality (based on curvature drift frequency) ---
    fprintf('... calculating Solomon/Maslov collisionality\n');
    % This calculates nu*_e = nu_ei / omega_De from first principles,
    % as defined in Maslov (2009) and used by Solomon (2010).
    c_s = sqrt(e * Te_mean ./ constants.plasma.m_ion); % Ion sound speed
    k_perp_rho_s = sqrt(0.1); % Gyrokinetic estimate [cite: 1605]
    omega_De = 2 * k_perp_rho_s .* c_s ./ R_coord; % Curvature drift frequency
    collisionality.nu_star_e_solomon = collisions.nu_ei_15 ./ omega_De;

    % --- 6. Gradients ---
    gradients = struct();
    gradients.R_over_Ln = R0 .* -gradient(log(ne_fine), r_fine);
