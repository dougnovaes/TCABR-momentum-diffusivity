...
%RUN_ADVANCED_FIT Performs inverse analysis to estimate neutral density.
    ...
    % Access from the 'variants' struct (Stage 7b)
    if isfield(variants, 'Solomon') && isfield(variants.Solomon, 'R0_profile')
        target_chi_profile = variants.Solomon.R0_profile.profile_avg;
        fprintf('Benchmark selected: Solomon (R0, Gradient Profiles)\n');
    else
        error('Benchmark field variants.Solomon.R0_profile not found.');
    end
    ...
    % Use metadata from variants if available
...
% %   the theoretical benchmark (Solomon with local gradients).
% 
%     arguments
%         velocity_results (1,1) struct
%         collision_profiles (1,1) struct
...
%     % We access this from the 'variants' struct computed in Stage 7b.
%     if isfield(variants, 'Solomon') && isfield(variants.Solomon, 'R0_profile')
%         target_chi_profile = variants.Solomon.R0_profile.profile_avg;
%         fprintf('Benchmark selected: Solomon (R0, Gradient Profiles)\n');
%     else
%         % Fallback for debugging structure issues
%         error('Benchmark field variants.Solomon.R0_profile not found.');
%     end
% 
%     % Sanitize Target
...
%     mask_range = (r_norm > 0.20) & (r_norm < 0.98);
% 
%     % Physics mask (avoid singularities where R/L_Vphi -> 0)
%     if isfield(variants, 'meta') && isfield(variants.meta, 'R_over_LVphi_profile')
%         R_over_LVphi = variants.meta.R_over_LVphi_profile;
%         mask_physics = abs(R_over_LVphi) > 1.0; 
%     else
...
% 
%     save(fullfile(results_dir, 'advanced_fit_results.mat'), 'results_adv');
%     fprintf('Results saved to: %s\n', fullfile(results_dir, 'advanced_fit_results.mat'));
% 
%     % --- 6. Plotting (Delegated) ---
%     if exist('plot_advanced_fit_results', 'file') == 2
%         plot_advanced_fit_results(results_adv, results_dir);
%     else
%         warning('Plotting function plot_advanced_fit_results not found. Plots skipped.');
%     end
% end
% 
% % --- Helpers ---
