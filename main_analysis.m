...
run_flags = struct( ...
    ...
    'physics_profiles',      false,  ... % <--- ATTENTION: TRUE (to calculate Solomon's collisionality)
    'theoretical_models',    false,  ... % <--- ATTENTION: TRUE (to  usar a nova colisionalidade)
    'neutral_and_collision', false, ... 
    'diffusivity_thesis',    false, ... 
    'diffusivity_scaling',   false,  ... % <--- ATTENTION: TRUE (para propagar a nova física)
    'build_variants',        false   ... % <--- ATTENTION: TRUE (para atualizar o benchmark)
);

plot_flags = struct( ...
    'justification_and_fits',  false, ...
    'thesis_method_results',   false, ...
    'supporting_profiles',     false, ...
    'final_comparison',        false,  ... % Ato 1: Mostra os modelos (com a singularidade física)
