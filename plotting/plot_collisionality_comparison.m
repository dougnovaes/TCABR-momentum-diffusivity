...
%   collisionality and their propagation into chi_phi and chi_eff.
    ...
    % If it wasn't calculated in stage 3, we can't plot it.
    if ~isfield(derived_profiles.collisionality, 'nu_star_e_solomon')
        warning('Solomon collisionality (nu_star_e_solomon) not found in derived_profiles. Skipping comparison plot.');
        return;
    end

    % --- 2. Recalculate Simple Theoretical Variants for Both Collisionalities ---
    % We use a simplified Solomon model (R0, mid-grads) just to show the sensitivity.
    ...
    % Re-estimate R/L_Vphi if not readily available in a simple struct
    ...
    % Data
    nu_wesson = derived_profiles.collisionality.nu_star_e_wesson;
    nu_solomon = derived_profiles.collisionality.nu_star_e_solomon;
    ...
    xlabel(ax3, 'r/a'); ylabel(ax3, '\chi_{\phi, eff} [m^2/s]');
    title(ax3, '(c) Effective Diffusivity Sensitivity');
    legend(ax3, 'Location', 'best');
    xlim(ax3, [0, 1]); ylim(ax3, [0, 20]); set_publication_style(ax3);
