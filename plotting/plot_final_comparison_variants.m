...
% --- END OF SECTION 1 ---
    ...
    for i = 1:numel(pinch_models)
        ...
        plot(ax, r_norm, diffusivity_exp.profile_avg, '-', 'LineWidth', 3.5, ...
            'Color', colors(1,:), 'DisplayName', '$χ_\varphi^{\mathrm{eff}}\ \text{(Exp.)}$');

        % --- 2b. Plot Pure Diffusivity Baseline ---
        plot(ax, r_norm, meta.chi_phi_solo_avg, ':', 'LineWidth', 2, 'Color', 'k', ...
            'DisplayName', '$χ_\varphi\ \text{(Solomon base)}$');
        ...
        for j = 1:numel(variant_keys)
            ...
            if isfield(model_data, key)
                ...
                % Extract the DisplayName template from the style properties
                ...
                if is_mid_variant
                    % For mid variants: insert numeric value
                    ...
                    % For profile variants: use template as-is (no numeric value)
                    display_name = display_name_template;
                end
        ...
        % Create legend at the top-center (with plain text interpreter)
        lgd = legend(ax, 'Location', 'north');
        set(lgd, 'Interpreter', 'latex'); % No LaTeX for legend
        % lgd.Title.String = 'Theoretical Variants';
        lgd.Box = 'off';
        
        set_publication_style(ax);
        
        % Create the theoretical formula annotation box (keeping LaTeX here)
